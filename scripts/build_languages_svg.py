...
def main():
    ...
    if not repos:
        print("⚠️  No repositories found!", file=sys.stderr)
        sys.exit(1)
    ...
    if not languages:
        print("⚠️  No language data found!", file=sys.stderr)
        sys.exit(1)
    ...
    print(
        f"✅ Top languages: {', '.join([f'{name} ({bytes_count:,} bytes)' for name, bytes_count in top_langs])}"
    )
